...
export const TabProvider = ({ children }) => {
    ...
    const openSubPage = useCallback((config) => {
        ...
        // SINGLE LIST TAB POLICY
        // If opening a LIST, close other lists first
        if (config.template === 'LIST' || config.type === 'LIST') {
            setTabs(prev => prev.filter(t => t.type !== 'LIST' && t.type !== 'DEFAULT'));
            // Note: 'DEFAULT' might often be the initial main list depending on how it's initialized, 
            // but checking 'LIST' is safer for explicit subpages. 
            // Adjusting this to only close explicitly marked LISTs to be safe.
            setTabs(prev => prev.filter(t => t.type !== 'LIST'));
        }
        ...
        const newTab = {
            ...
            type: config.template || 'LIST', // Defaulting to LIST if undefined for Sidebar clicks? No, use config.
            isFixed: false,
            data: config.data || {} // Form draft data
        };

        // Re-apply filter logic inside the setter to be atomic
        setTabs(prev => {
            const isNewList = (config.template === 'LIST' || config.type === 'LIST');
            const cleanTabs = isNewList ? prev.filter(t => t.type !== 'LIST') : prev;
            return [...cleanTabs, newTab];
        });
        ...
        if (location.pathname.startsWith('/app/')) {
            setTabs(prev => {
                ...
                if (newTabs[mainTabIndex]) {
                    ...
                    let newTitle = 'Carregando...';
                    try {
                        const groups = mockEngine.getNavigation();
                        const foundPage = groups.flatMap(g => g.pages).find(p => location.pathname.includes(p.id));
                        if (foundPage) newTitle = foundPage.name;
                        // Special case for Dashboard
                        if (location.pathname.includes('dashboard')) newTitle = 'Dashboard';
                    } catch (e) { console.warn("Title Resolve Fail", e); }
