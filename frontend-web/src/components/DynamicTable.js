...
const DynamicTable = ({ entity, onEdit, onCreate, onDelete }) => {
    ...
    // 2. Fetch Data
    const { data: records, isLoading: isDataLoading, refetch } = useQuery({
        queryKey: ['data', entity],
        queryFn: async () => {
            const response = await apiClient.get(`/v1/engine/${entity}`);
            return response.data;
        }
    });
    ...
    const columns = meta.fields.filter(f => !f.is_hidden_in_list); // You could add this flag later
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs uppercase font-semibold">
                        <tr>
                            {columns.map(col => (
                    ...
                    <tbody className="divide-y dark:divide-gray-700">
                        {records?.map((row, idx) => (
                            <tr key={row.id || idx} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                {columns.map(col => (
