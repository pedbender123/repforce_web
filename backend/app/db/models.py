...
class User(Base):
    """
    Modelo de Usuário (SysAdmin, Admin ou Representante).
    """
    __tablename__ = "users"
    id = Column(Integer, primary_key=True, index=True)
    
    # --- MUDANÇAS AQUI ---
    username = Column(String, unique=True, index=True, nullable=True) # Para login
    email = Column(String, unique=False, index=True, nullable=True) # Para contato
    # --- FIM DA MUDANÇA ---

    hashed_password = Column(String, nullable=False)
    profile = Column(String, nullable=False) # 'sysadmin', 'admin', 'representante'
